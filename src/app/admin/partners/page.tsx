  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-slate-900">{t("Холбоос ба түншүүд удирдах", "Manage Links & Partners", "リンクとパートナー管理", "管理链接与伙伴")}</h2>
        <button
          onClick={() => {
            setEditingId(null);
            setForm(EMPTY);
      ...
      <p className="text-sm text-slate-500 mb-6 max-w-2xl">
        {t(
          ...
          "在此添加的内容将显示在首页的「链接与伙伴」区块——例如姊妹俱乐部。"
        )}
      </p>
      ...
      {showForm && (
        <form onSubmit={submit} className="rounded-xl border border-slate-200 p-6 mb-8 grid gap-3">
          <div className="grid gap-3 sm:grid-cols-2">
            <input required placeholder={t("Нэр", "Name", "名前", "名称")} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
      ...
      {items && items.length === 0 && <p className="text-slate-400 text-sm">{t("Жагсаалт хоосон байна.", "Nothing added yet.", "まだ登録されていません。", "暂无记录。")}</p>}
