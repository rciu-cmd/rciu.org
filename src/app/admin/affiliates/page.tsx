...
export default function AdminAffiliatesPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (logoFile) {
      ...
      const path = `logos/affiliates/${Date.now()}-${safeName}`;
  ...
  async function remove(item: AffiliateRow) {
    if (!confirm(t("Устгах уу?", "Delete this club?", "削除しますか?", "確定刪除嗎?"))) return;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-slate-900">{t("Дэмждэг клубууд удирдах", "Manage Sponsored Clubs", "スポンサークラブ管理", "贊助俱樂部管理")}</h2>
        <button
          onClick={() => {
            setEditingId(null);
            setForm(EMPTY);
        ...
        >
          {showForm ? t("Хаах", "Cancel", "キャンセル", "取消") : t("+ Шинэ клуб", "+ New Club", "+ 新規クラブ", "+ 新建俱樂部")}
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={submit} className="rounded-xl border border-slate-200 p-6 mb-8 grid gap-3">
          <div className="grid gap-3 sm:grid-cols-2">
            <input required placeholder={t("Клубын нэр", "Club name", "クラブ名", "俱樂部名稱")} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <input type="date" placeholder={t("Дүрэмт болсон огноо", "Chartered date", "認可日", "註冊日期")} value={form.chartered_date} onChange={(e) => setForm({ ...form, chartered_date: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
            <input type="number" min="0" placeholder={t("Гишүүдийн тоо", "Member count", "会員數", "會員人數")} value={form.member_count} onChange={(e) => setForm({ ...form, member_count: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <p className="text-sm font-semibold text-slate-700 mb-1">{t("Лого", "Logo", "ロゴ", "徽標")}</p>
          ...
          <p className="text-sm font-semibold text-slate-700 mt-2">{t("Холбоо барих (тэргүүн)", "Contact (club president)", "連絡先(会長)", "聯繫方式(社長)")}</p>
          <div className="grid gap-3 sm:grid-cols-3">
            <input placeholder={t("Тэргүүний нэр", "President's name", "会長名", "社長姓名")} value={form.president_name} onChange={(e) => setForm({ ...form, president_name: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
            <input placeholder={t("Утас", "Phone", "電話", "電話")} value={form.contact_phone} onChange={(e) => setForm({ ...form, contact_phone: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
            <input type="email" placeholder={t("И-мэйл", "Email", "メール", "郵箱")} value={form.contact_email} onChange={(e) => setForm({ ...form, contact_email: e.target.value })} className="rounded-md border border-slate-300 px-3 py-2 text-sm" />
          </div>
      ...
      {items && items.length === 0 && <p className="text-slate-400 text-sm">{t("Клуб алга.", "No sponsored clubs yet.", "クラブがありません。", "暫無贊助俱樂部。")}</p>}
